...
eval :: ExprT -> Integer
eval (ExprT.Add c@(_) (Lit x)) = x + eval c
eval (ExprT.Mul c@(_) (Lit x)) = x * eval c
eval (Lit x) = x
...
evalStr x = case xs of (Just d) -> Just (eval d)
                       _ -> Nothing  
         where xs = parseExp Lit ExprT.Add ExprT.Mul x
{- Exercise 3 -}
class Expr a where
    lit :: Integer -> a 
    add :: a  -> a  -> a
...
instance Expr ExprT where
    lit = Lit
    add x1  x2 = ExprT.Add x1  x2
    mul x1 x2 = ExprT.Mul  x1  x2
